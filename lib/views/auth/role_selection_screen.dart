...
/// Satu screen pemilihan peran untuk dua use-case:
/// - [isExistingAuthUser] == false → calon akun baru, lanjut ke RegisterScreen(role).
/// - [isExistingAuthUser] == true  → user sudah ter-authenticate (mis. Google
///   new-user tanpa role), simpan role ke uid yang sama via selectRole().
class RoleSelectionScreen extends StatefulWidget {
  final bool isExistingAuthUser;

  const RoleSelectionScreen({super.key, this.isExistingAuthUser = false});
...
class _RoleSelectionScreenState extends State<RoleSelectionScreen> {
  ...
  Future<void> _onRolePicked(String role) async {
    if (_busy) return;

    // Calon akun baru → lanjut ke form pendaftaran dgn role terpilih.
    ...
    // User sudah authenticated (Google) → update role pada uid yang sama.
    setState(() => _busy = true);
